...
function PairSums(arr) {
    ...
    let best = [arr[0] * arr[1], arr[0] + arr[1], 0]; // total value, sum of elements of subarray, starting index of subarray
    let prev = best;
    for (let i = 2; i < arr.length; i++) {
        let local_value = arr[i] * arr[i - 1];
        let accumulated_value = prev[1] * arr[i] + prev[0];
        let current;
